#!/bin/sh
#
# Plesk chroot template for CentOS 7 by QWeb Ltd
#
# On CentOS 6, setting a domains Web Hosting Access -> Access to the server over SSH value to "/bin/bash (chrooted)" within Plesk just worked, but on CentOS 7 this causes PHP scripts set to run as cron tasks to break for a variety of reasons.
#
# This script configures the chroot template Plesk uses to resolve the various problems we've come across at QWeb:
#     - The PHP 5.6 to 7.4 binaries provided by Plesk are made accessible within the chroot environment.
#     - DNS resolution is fixed within the chroot environment.
#     - MySQL is made accessible within the chroot environment, and to PHP, though mysqli_connect() only works with 127.0.0.1, not localhost.
...
# Ref https://support.plesk.com/hc/en-us/articles/213909545-How-to-add-programs-to-a-chrooted-shell-environment-template-in-Plesk
...
# Adds MySQL to the environment to fix PHPs mysql and mysqli functions. This appears to be the same binary if using MariaDB too.
